...
void sendData(float temperature, float absolute_pressure, float pressure) {
  ...
  if (client) {
...

    {
      ...
      if (https.begin(*client, observatory_url.c_str())) {  // HTTPS
        ...
        String body = "{ \"bucket\": \"barometer-living-room\", \"payload\": { \"temperature\": " + String(temperature) + ", \"absolute_pressure\": " + String(absolute_pressure) + ", \"pressure\": " + String(pressure) + " } }";
...
void loop() {
  if (bmp.takeForcedMeasurement()) {
    ...
    sendData(temperature, abs_pressure, pressure);
  } else {
    Serial.printf("BMP280 measurement not ready\n");
  }
